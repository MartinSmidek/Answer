...
CREATE TABLE `objednavka` (`id_order` int(11) unsigned, `id_akce` int(11), `DS2024` text, `kod_akce` smallint(6), `adults` tinyint(4) unsigned, `kids_10_15` tinyint(4) unsigned, `kids_3_9` tinyint(4) unsigned, `kids_3` tinyint(4) unsigned, `board` tinyint(4) unsigned,`nazev` tinytext, `note` text, `jmeno` tinytext, `prijmeni` varchar(80), `ulice` tinytext, `psc` varchar(10), `obec` varchar(50), `telefon` varchar(20), `email` varchar(80), `org` tinytext, `ic` int(11), `dic` varchar(20), `od` date, `do` date);
DROP TABLE IF EXISTS `objednavka`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `objednavka` AS select `setkani4_test`.`tx_gnalberice_order`.`uid` AS `id_order`,`setkani4_test`.`tx_gnalberice_order`.`id_akce` AS `id_akce`,`setkani4_test`.`tx_gnalberice_order`.`DS2024` AS `DS2024`,`setkani4_test`.`tx_gnalberice_order`.`akce` AS `kod_akce`,`setkani4_test`.`tx_gnalberice_order`.`adults` AS `adults`,`setkani4_test`.`tx_gnalberice_order`.`kids_10_15` AS `kids_10_15`,`setkani4_test`.`tx_gnalberice_order`.`kids_3_9` AS `kids_3_9`,`setkani4_test`.`tx_gnalberice_order`.`kids_3` AS `kids_3`,`setkani4_test`.`tx_gnalberice_order`.`board` AS `board`,`setkani4_test`.`tx_gnalberice_order`.`nazev` AS `nazev`,`setkani4_test`.`tx_gnalberice_order`.`note` AS `note`,`setkani4_test`.`tx_gnalberice_order`.`firstname` AS `jmeno`,`setkani4_test`.`tx_gnalberice_order`.`name` AS `prijmeni`,`setkani4_test`.`tx_gnalberice_order`.`address` AS `ulice`,`setkani4_test`.`tx_gnalberice_order`.`zip` AS `psc`,`setkani4_test`.`tx_gnalberice_order`.`city` AS `obec`,`setkani4_test`.`tx_gnalberice_order`.`telephone` AS `telefon`,`setkani4_test`.`tx_gnalberice_order`.`email` AS `email`,`setkani4_test`.`tx_gnalberice_order`.`org` AS `org`,`setkani4_test`.`tx_gnalberice_order`.`ic` AS `ic`,`setkani4_test`.`tx_gnalberice_order`.`dic` AS `dic`,cast(from_unixtime(`setkani4_test`.`tx_gnalberice_order`.`fromday`) as date) AS `od`,cast(from_unixtime(`setkani4_test`.`tx_gnalberice_order`.`untilday`) as date) AS `do` from `setkani4_test`.`tx_gnalberice_order`;
