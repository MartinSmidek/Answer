...
function akce2_vzorec_test($id_akce,$hnizdo=0,$nu=2,$nD,$nd=0,$nk=0,$np=0,$table_class='') {  trace();
...
//                                                 display("i.par.sel=$par->sel");
  ...
  $qry=  "
    SELECT
      p.pouze,p.poznamka,/*p.platba - není atribut osoby!,*/p.funkce,p.skupina,p.pokoj,s.s_role,
...
function akce2_stravenky($akce,$par,$title,$vypis,$export=false,$hnizdo=0) { trace();
  global $diety,$diety_,$jidlo_;
...
//   $diety= array('','_bm','_bl');                             -- globální nastavení
  foreach ($diety as $i=>$d) {
    ...
    $par->dieta= $d;
    $res= akce2_stravenky_diety($akce,$par,"$title {$diety_[$d]}","$vypis$d",$export,$hnizdo);
    $res->dieta= $d;
...
function akce2_stravenky_diety($akce,$par,$title,$vypis,$export=false,$hnizdo=0) { trace();
...
function akce2_vyuctov_pary($akce,$par,$title,$vypis,$export=false) { trace();
  global $tisk_hnizdo;
  $ord= $par->ord ? $par->ord : "IF(funkce<=2,1,IF(funkce IN (10,14),3,2)),_jm";
  $result= (object)array();
  $tit= "Manželé:25"
      // . ",id_pobyt"
...
#   'U','U1','U2','U3','U4','U5' - rozeslat účastníkům akce dopis.id_duakce ukazující do akce
