...
# -------------------------------------------------------------------------------------------------- lide_family
function lide_family($par) { trace();
  $html= "";
  $n= 0;
  switch ( $par->fce ) {
  // ----------------------------------------- 0. init tabulky
  case 0:
    foreach(explode(',','akce,pobyt,spolu,osoba,rodina,tvori') as $tab) {
      $qry= "TRUNCATE TABLE $tab";
      $res= mysql_qry($qry);
      if ( $res ) $html.= "<br>$tab vyprázdněna";
    }
    $html.= "<hr>";
    break;
  // ----------------------------------------- 1. akce
  case 1:
    foreach(explode(',','akce') as $tab) {
      $qry= "TRUNCATE TABLE $tab";
      $res= mysql_qry($qry);
      if ( $res ) $html.= "<br>$tab vyprázdněna";
    }
    $html.= "<hr>";
    // znovu
    $qryp= "SELECT * FROM ms_akce AS m ";
    $resp= mysql_qry($qryp);
    $n= 0;
    while ( $resp && $p= mysql_fetch_object($resp) ) {
      $qryu= "INSERT INTO akce (id_duakce,typ,spec,nazev,datum_od,datum_do,note,misto,druh,poradatel)
        VALUES ({$p->id_akce},10,0,'{$p->nazev}','{$p->datum_od}','{$p->datum_do}','F',
        '{$p->misto}','{$p->druh}','{$p->poradatel}')";
      $resu= mysql_qry($qryu);
      $n+= mysql_affected_rows();
    }
    $html.= "<br>akce - $n záznamů";
    break;
  // ----------------------------------------- 2. páry: osoba, tvori, rodina
  case 2:
    foreach(explode(',','osoba,rodina,tvori') as $tab) {
      $qry= "TRUNCATE TABLE $tab";
      $res= mysql_qry($qry);
      if ( $res ) $html.= "<br>$tab vyprázdněna";
    }
    ...
    // ms_pary
    $nr= $no= 0;
    $qryp= "SELECT * FROM ms_pary WHERE 1";
    $resp= mysql_qry($qryp);
    while ( $resp && $p= mysql_fetch_assoc($resp) ) {
      $nr++;
      $p["id_dupary"]= $p["id_pary"];
      $r= rodina_insert($p);
      if ( $p['jmeno_m'] ) {
      ...
      if ( !$p['jmeno_m'] && !$p['jmeno_z'] )        // 12 případů
        $html.= "neosoba id_pary={$p['id_pary']}, ";
//                                                       break;
    }
    $html.= "<br>rodina - $nr záznamů";
    $html.= "<br>tvori, osoba - $no záznamů";
    break;
  // ----------------------------------------- 3. děti: osoba, tvori, rodina
  case 3:
    // zrušení staré verze
    ...
    // ms_deti
    $no= 0;
    $qryd= "SELECT * FROM ms_deti ";
    $resd= mysql_qry($qryd);
    while ( $resd && $d= mysql_fetch_assoc($resd) ) {
      $d["id_dupary"]= $d["id_pary"];
      $d["id_dudeti"]= $d["id_deti"];
  ...
  case 4:
    // zrušení staré verze
    foreach(explode(',','pobyt,spolu') as $tab) {
      $qry= "TRUNCATE TABLE $tab";
      $res= mysql_qry($qry);
      if ( $res ) $html.= "<br>$tab vyprázdněna";
    }
    ...
    $n1= $n2= 0;
    $qryd= "SELECT * FROM ms_kurs ";
    $resd= mysql_qry($qryd);
    while ( $resd && $k= mysql_fetch_object($resd) ) {
      $k->id_dukurs= $k->id_kurs;
      $k->id_duakce= $k->id_akce;
      $np++;
      $ids= $vals= ''; $del= '';
      foreach(explode(',',"id_dukurs,skupina,funkce,aktivita,
        ...
        $id= trim($id);
        $ids.= "$del$id";
        $vals.= "$del'".mysql_real_escape_string($k->$id)."'";
        $del= ',';
      }
      ...
      switch ($k->pouze) {
      case 0: // oba
        $o= select("id_osoba","osoba","id_dupary={$k->id_pary} AND sex=1");
        if ( $o ) {
          bydli_insert($id_pobyt,$o);
          $ns++;
        }
        else
          $html.= "<br>spor v pouze pro id_dukurs={$k->id_kurs} /oba-muž není";
        $o= select("id_osoba","osoba","id_dupary={$k->id_pary} AND sex=2");
        if ( $o ) {
          bydli_insert($id_pobyt,$o);
          $ns++;
        }
        else
          $html.= "<br>spor v pouze pro id_dukurs={$k->id_kurs} /oba-žena není";
        break;
      case 1: // jen muž
      case 2: // jen žena
        $o= select("id_osoba","osoba","id_dupary={$k->id_pary} AND sex={$k->pouze}");
        if ( $o ) {
          bydli_insert($id_pobyt,$o);
          $ns++;
        }
        else
          $html.= "<br>spor v pouze pro id_dukurs={$k->id_kurs} ";
        break;
      }
...
function rodina_insert($p,$origin='p') {  trace();
  ...
  $svatba=   $p["svatba"];
  $note=     mysql_real_escape_string(strtr($p["poznamka"],"'","’"));
...
function osoba_insert($p,$sex,$_m,$orig) {
  $id_dupary=$p["id_dupary"];
  $jmeno=    mysql_real_escape_string($p["jmeno$_m"]);
  $prijmeni= mysql_real_escape_string($p["prijmeni$_m"] ? $p["prijmeni$_m"] : $p["jmeno"]);
  $ulice   = strtr($p["adresa"],"'","’");
  $psc     = $p["psc"];
  $os=       $p["mesto"];
  list($o,$s)= explode(',',$os);
  ...
  $zamest  = mysql_real_escape_string($p["zamest$_m"]);
  $zajmy   = mysql_real_escape_string($p["zajmy$_m"]);
  $jazyk   = $p["jazyk$_m"];
  $cirkev  = $p["cirkev$_m"];
  $aktivita= mysql_real_escape_string($p["aktivita$_m"]);
  $clen    = $p["clen$_m"];
  $historie= '';
  $qryo= "INSERT INTO osoba (
          id_dupary,jmeno,prijmeni,ulice,psc,obec,stat,narozeni,rc_xxxx,
